...
```tql
chart_line x=field, y=any, [x_min=any, x_max=any, resolution=duration,
            x_log=bool, y_log=bool, group=any]
```
...
```tql
metrics "publish"
chart_line x=timestamp, y=sum(events), x_min=now()-1d, group=pipeline_id, resolution=30min
```
