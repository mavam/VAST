...
public:
  ...
  auto make_function(invocation inv,
    ...
    auto header_arg = ast::expression{};
    parser.positional("input", input, "string");
    parser.named("header", header_arg, "string|list<string>");
    auto opt_parser = xsv_common_parser_options_parser{
      name(),
      std::string{Sep},
      std::string{ListSep},
    ...
    TRY(auto header_data, const_eval(header_arg, ctx));
    auto fields = std::vector<std::string>{};
    match(
      header_data,
      [&](const std::string& s) {
        fields = parse_header(s, opts, quoting_options, ctx);
      },
      [&](const list& l) {
        if (l.empty()) {
          // TODO: warn?
      ...
      [&](const auto& unexpected) {
        ...
        diagnostic::error("`header` must be a `string` or `list<string>`")
          .primary(header_arg, "got `{}`", t ? t->kind() : type_kind{})
          .emit(ctx);
      });
